...
const Advice: React.FC<{ advice: advice; majorId: string }> = ({
  advice,
  majorId,
}) => {
      ...
      <IonCardContent>
        <IonGrid>
          <IonRow className={classes.rightAlign}>
            ...
            <IonCol className={classes.Col + " " + classes.Color}>
              <IonRow>
                <IonCol>صعوبة التخصص</IonCol>
                <IonCol>{advice.rating[0]}</IonCol>
              </IonRow>
              <IonRow>
                <IonCol>كفائة الطاقم التدريسي</IonCol>
                <IonCol>{advice.rating[1]}</IonCol>
              </IonRow>
              <IonRow>
                <IonCol>فرص العمل</IonCol>
                <IonCol>{advice.rating[2]}</IonCol>
              </IonRow>
              <IonRow>
                <IonCol>المواصلات</IonCol>
                <IonCol>{advice.rating[3]}</IonCol>
              </IonRow>
            </IonCol>
          </IonRow>
          <IonRow>
            <IonCol className={classes.Col}>
              <IonIcon
                icon={caretUpSharp}
                className={classes.Icon + " " + upSelected}
                onClick={(e) => context.handleUpVote(advice.userId, majorId)}
              />
              <span className={classes.Voting}>{votes}</span>
              <IonIcon
                icon={caretDownSharp}
                className={classes.Icon + " " + downSelected}
                onClick={(e) => context.handleDownVote(advice.userId, majorId)}
              />
            </IonCol>
          </IonRow>
        </IonGrid>
